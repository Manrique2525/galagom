...
export default function CoverageMap() {
  ...
  return <svg className="h-auto w-full" viewBox="0 0 500 300" fill="none" role="img" aria-label="Rutas abstractas desde Cancún hacia Holbox, Isla Mujeres y Cozumel">
    ...
    {routes.map((route) => <motion.path key={route} d={route} stroke="#A9B9D8" strokeWidth="1.5" strokeDasharray="5 7" initial={reducedMotion ? { pathLength: 1, opacity: 1 } : { pathLength: 0, opacity: 0.3 }} whileInView={reducedMotion ? undefined : { pathLength: 1, opacity: 1 }} viewport={{ once: true, amount: 0.4 }} transition={{ duration: 0.9, ease: "easeInOut" }} />)}
