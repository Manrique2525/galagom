...
export default function QuoteForm() {
  ...
  return <form className="space-y-6" onSubmit={handleSubmit(onSubmit)} noValidate>
    ...
    <div className="grid gap-6 sm:grid-cols-2"><Field id="name" label="Nombre" error={getError("name")} register={register} /><Field id="phone" label="Teléfono" error={getError("phone")} register={register}><input id="phone" className={inputClass} type="tel" inputMode="tel" autoComplete="tel" maxLength={25} aria-invalid={Boolean(errors.phone)} aria-describedby={errors.phone ? "phone-error" : undefined} {...register("phone")} /></Field></div>
    ...
    <div className="grid gap-6 sm:grid-cols-2"><Field id="service" label="Tipo de servicio" error={getError("service")} register={register}><select id="service" className={inputClass} defaultValue="" aria-invalid={Boolean(errors.service)} aria-describedby={errors.service ? "service-error" : undefined} {...register("service")}><option value="" disabled>Selecciona una opción</option>{quoteServices.map((service) => <option value={service} key={service}>{service}</option>)}</select></Field><Field id="date" label="Fecha estimada del servicio (opcional)" error={getError("date")} register={register}><input id="date" className={inputClass} type="date" aria-invalid={Boolean(errors.date)} aria-describedby={errors.date ? "date-error" : undefined} {...register("date")} /></Field></div>
    <div className="grid gap-6 sm:grid-cols-2"><Field id="origin" label="Origen" error={getError("origin")} register={register}><input id="origin" className={inputClass} placeholder="Ej. Cancún, Quintana Roo" autoComplete="address-level2" maxLength={150} aria-invalid={Boolean(errors.origin)} aria-describedby={errors.origin ? "origin-error" : undefined} {...register("origin")} /></Field><Field id="destination" label="Destino" error={getError("destination")} register={register}><input id="destination" className={inputClass} placeholder="Ej. Cozumel, Quintana Roo" autoComplete="address-level2" maxLength={150} aria-invalid={Boolean(errors.destination)} aria-describedby={errors.destination ? "destination-error" : undefined} {...register("destination")} /></Field></div>
    ...
    <div className="space-y-4 border-t border-border pt-6"><p className="text-sm leading-6 text-text-muted">Al enviar tus datos, aceptas que GALAGOM los utilice para atender tu solicitud. El aviso de privacidad definitivo requiere validación legal.</p><div className="flex flex-col items-start gap-4 sm:flex-row sm:items-center"><Button type="submit" size="large" disabled={isSubmitting}>{isSubmitting ? "Enviando..." : "Solicitar cotización"}<span className="ml-3" aria-hidden="true">↗</span></Button><SubmitStatus status={status} message={errorMessage} /></div></div>
  </form>;
}
