...
function Field({ id, label, error, register, children }: { id: keyof QuoteFormData; label: string; error?: string; register: UseFormRegister<QuoteFormData>; children?: ReactNode }) {
  ...
  return <div><label className="mb-2 block text-sm font-bold text-text" htmlFor={id}>{label}</label>{children ?? <input id={id} className={inputClass} autoComplete={id === "name" ? "name" : undefined} aria-invalid={Boolean(error)} aria-describedby={error ? errorId : undefined} {...register(id)} /> }<FieldError id={errorId} message={error} /></div>;
}
...
export default function QuoteForm() {
  ...
  return <form className="space-y-6" onSubmit={handleSubmit(onSubmit)} noValidate>
    <div className="grid gap-6 sm:grid-cols-2">
      ...
      <Field id="phone" label="Teléfono" error={getError("phone")} register={register}><input id="phone" className={inputClass} type="tel" inputMode="tel" autoComplete="tel" aria-invalid={Boolean(errors.phone)} aria-describedby={errors.phone ? "phone-error" : undefined} {...register("phone")} /></Field>
    </div>
    <Field id="email" label="Correo electrónico" error={getError("email")} register={register}><input id="email" className={inputClass} type="email" inputMode="email" autoComplete="email" aria-invalid={Boolean(errors.email)} aria-describedby={errors.email ? "email-error" : undefined} {...register("email")} /></Field>
    <div className="grid gap-6 sm:grid-cols-2">
      ...
      <Field id="date" label="Fecha estimada del servicio (opcional)" error={getError("date")} register={register}><input id="date" className={inputClass} type="date" aria-describedby={errors.date ? "date-error" : undefined} {...register("date")} /></Field>
    </div>
    <div className="grid gap-6 sm:grid-cols-2">
      <Field id="origin" label="Origen" error={getError("origin")} register={register}><input id="origin" className={inputClass} placeholder="Ej. Cancún, Quintana Roo" autoComplete="address-level2" aria-invalid={Boolean(errors.origin)} aria-describedby={errors.origin ? "origin-error" : undefined} {...register("origin")} /></Field>
      <Field id="destination" label="Destino" error={getError("destination")} register={register}><input id="destination" className={inputClass} placeholder="Ej. Cozumel, Quintana Roo" autoComplete="address-level2" aria-invalid={Boolean(errors.destination)} aria-describedby={errors.destination ? "destination-error" : undefined} {...register("destination")} /></Field>
    </div>
    <Field id="description" label="¿Qué necesitas transportar o almacenar?" error={getError("description")} register={register}><textarea id="description" className={`${inputClass} min-h-32 resize-y`} placeholder="Describe brevemente tu mercancía, volumen aproximado o cualquier detalle importante." aria-invalid={Boolean(errors.description)} aria-describedby={errors.description ? "description-error" : undefined} {...register("description")} /></Field>
