...
export default function ServiceCard({ service, featured = false, dark = false, side = false }: { service: Service; featured?: boolean; dark?: boolean; side?: boolean }) {
  ...
  return <article className={`group relative isolate flex min-h-[400px] flex-col justify-between overflow-hidden rounded-[var(--card-radius)] border p-6 transition-all duration-[var(--motion-base)] hover:border-primary-soft hover:shadow-[var(--shadow-soft)] sm:p-8 ${featured ? "lg:min-h-full" : ""} ${immersive ? "border-primary bg-primary-dark" : "border-border bg-white"}`}>
    {service.image && immersive && <Image src={service.image} alt={service.imageAlt ?? ""} fill sizes="(min-width: 1024px) 60vw, 100vw" className="-z-10 object-cover opacity-65 transition-transform duration-700 group-hover:scale-[1.025]" />}
    ...
    {service.image && !immersive && <div className={`relative -mx-6 -mt-6 mb-7 h-52 overflow-hidden sm:-mx-8 sm:-mt-8 sm:mb-8 ${side ? "lg:absolute lg:inset-y-0 lg:right-0 lg:mb-0 lg:mt-0 lg:mx-0 lg:h-full lg:w-[55%]" : ""}`}><Image src={service.image} alt={service.imageAlt ?? ""} fill sizes="(min-width: 1024px) 45vw, 100vw" className="object-cover transition-transform duration-700 group-hover:scale-[1.025]" /><div className={`absolute inset-0 ${side ? "bg-gradient-to-r from-white via-white/10 to-transparent lg:from-white lg:via-white/20" : "bg-primary-dark/20"}`} /></div>}
