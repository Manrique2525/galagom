# Reporte de integridad de enlaces y navegación

- Fecha: 2026-09-12T18:17:45.258Z
- BASE_URL: http://localhost:3001
- Modo: production (next build + next start)

## Resumen

| Métrica | Valor |
| --- | --- |
| Enlaces auditados | 45 |
| Internos (página) | 11 |
| Anclas same-page | 27 |
| Anclas cross-page | 0 |
| Redirects | 0 |
| Externos HTTPS | 0 |
| WhatsApp | 3 |
| tel: | 4 |
| mailto: | 0 |
| Inválidos | 0 |
| Enlaces rotos | 0 |
| IDs duplicados | 0 |
| Errores de consola | 0 |
| Errores de página | 0 |
| Fallos red (first-party) | 0 |
| HTTP 4xx/5xx first-party | 0 |
| Errores de hidratación | 0 |

## Enlaces rotos

Sin enlaces rotos.

## Advertencias

- keyboard: skip-link no focuseado en 15 tabs

## Tabla de enlaces

| Source | Label | Destination | Type | Status | Notes |
| --- | --- | --- | --- | --- | --- |
| / | Saltar al contenido | #main-content | samePageFragment | OK |  |
| / | GALAGOM, ir al inicio | / | internalPage | OK |  |
| / | Inicio | / | internalPage | OK |  |
| / | Servicios | /#servicios | samePageFragment | OK |  |
| / | Cobertura | /#cobertura | samePageFragment | OK |  |
| / | Nosotros | /#nosotros | samePageFragment | OK |  |
| / | Contacto | /#cotizar | samePageFragment | OK |  |
| / | Cotizar flete | /#cotizar | samePageFragment | OK |  |
| / | Cotizar un flete ↗ | /#cotizar | samePageFragment | OK |  |
| / | Conocer servicios ↓ | #servicios | samePageFragment | OK |  |
| / | SOLICITAR SERVICIO → | /#cotizar | samePageFragment | OK |  |
| / | SOLICITAR SERVICIO → | /#cotizar | samePageFragment | OK |  |
| / | SOLICITAR SERVICIO → | /#cotizar | samePageFragment | OK |  |
| / | SOLICITAR SERVICIO → | /#cotizar | samePageFragment | OK |  |
| / | SOLICITAR SERVICIO → | /#cotizar | samePageFragment | OK |  |
| / | Cotizar una ruta ↗ | /#cotizar | samePageFragment | OK |  |
| / | llamar al 998 222 5373 | tel:+529982225373 | tel | OK |  |
| / | Aviso de privacidad | /privacidad | internalPage | OK |  |
| / | 998 222 5373 | tel:+529982225373 | tel | OK |  |
| / | Aviso de privacidad | /privacidad | internalPage | OK |  |
| / | Escríbenos por WhatsApp | https://wa.me/529982225373?text=Hola%2C%20me%20gustar%C3%ADa%20solicitar%20informaci%C3%B3n%20y%20una%20cotizaci%C3%B3n%20sobre%20los%20servicios%20de%20GALAGOM. | whatsapp | OK |  |
| /privacidad | Saltar al contenido | #main-content | samePageFragment | OK |  |
| /privacidad | GALAGOM, ir al inicio | / | internalPage | OK |  |
| /privacidad | Inicio | / | internalPage | OK |  |
| /privacidad | Servicios | /#servicios | samePageFragment | OK |  |
| /privacidad | Cobertura | /#cobertura | samePageFragment | OK |  |
| /privacidad | Nosotros | /#nosotros | samePageFragment | OK |  |
| /privacidad | Contacto | /#cotizar | samePageFragment | OK |  |
| /privacidad | Cotizar flete | /#cotizar | samePageFragment | OK |  |
| /privacidad | 998 222 5373 | tel:+529982225373 | tel | OK |  |
| /privacidad | Aviso de privacidad | /privacidad | internalPage | OK |  |
| /privacidad | Escríbenos por WhatsApp | https://wa.me/529982225373?text=Hola%2C%20me%20gustar%C3%ADa%20solicitar%20informaci%C3%B3n%20y%20una%20cotizaci%C3%B3n%20sobre%20los%20servicios%20de%20GALAGOM. | whatsapp | OK |  |
| /404 | Saltar al contenido | #main-content | samePageFragment | OK |  |
| /404 | GALAGOM, ir al inicio | / | internalPage | OK |  |
| /404 | Inicio | / | internalPage | OK |  |
| /404 | Servicios | /#servicios | samePageFragment | OK |  |
| /404 | Cobertura | /#cobertura | samePageFragment | OK |  |
| /404 | Nosotros | /#nosotros | samePageFragment | OK |  |
| /404 | Contacto | /#cotizar | samePageFragment | OK |  |
| /404 | Cotizar flete | /#cotizar | samePageFragment | OK |  |
| /404 | Volver al inicio | / | internalPage | OK |  |
| /404 | Solicitar cotización | /#cotizar | samePageFragment | OK |  |
| /404 | 998 222 5373 | tel:+529982225373 | tel | OK |  |
| /404 | Aviso de privacidad | /privacidad | internalPage | OK |  |
| /404 | Escríbenos por WhatsApp | https://wa.me/529982225373?text=Hola%2C%20me%20gustar%C3%ADa%20solicitar%20informaci%C3%B3n%20y%20una%20cotizaci%C3%B3n%20sobre%20los%20servicios%20de%20GALAGOM. | whatsapp | OK |  |
